...
#### Responses WebSocket 传输（可选辅助） {#responses-websocket-transport-optional-helper}
...
##### 模式 1：不使用 session helper（可用） {#pattern-1-no-session-helper-works}
...
##### 模式 2：使用 `responses_websocket_session()`（推荐用于多轮复用） {#pattern-2-use-responses_websocket_session-recommended-for-multi-turn-reuse}
...
#### 常见运行配置目录 {#common-run-config-categories}

使用 `RunConfig` 可在不修改每个智能体定义的前提下覆盖单次运行行为。

##### 模型、provider 与 session 默认值 {#model-provider-and-session-defaults}
...
##### 安全防护措施、任务转移与模型输入塑形 {#guardrails-handoffs-and-model-input-shaping}
...
##### 工具审批与工具错误行为 {#tool-approval-and-tool-error-behavior}
...
## 状态与会话管理 {#state-and-conversation-management}
...
### 会话/聊天线程 {#conversationschat-threads}
...
#### 手动会话管理 {#manual-conversation-management}
...
#### 使用 Sessions 的自动会话管理 {#automatic-conversation-management-with-sessions}
...
#### 服务端管理会话 {#server-managed-conversations}
...
### 调用模型输入过滤器 {#call-model-input-filter}
...
## 持久化执行集成与 human-in-the-loop {#durable-execution-integrations-and-human-in-the-loop}
