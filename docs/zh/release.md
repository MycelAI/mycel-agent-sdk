...
search:
  exclude: true
---
# 发布流程/变更日志 {#release-processchangelog}
...
-   Chat Completions 集成现在可通过 `should_replay_reasoning_content` 选择启用推理内容回放，从而改善第三方 Chat Completions 适配器（例如 DeepSeek）在特定提供方上的推理/工具调用连续性。
...
此次次版本发布**不**引入破坏性变更。主要功能新增请查看上游 [OpenAI Agents SDK v0.12.0 发布说明](https://github.com/openai/openai-agents-python/releases/tag/v0.12.0)（本 fork 所追踪的上游版本线）。

### 0.11.0 {#0110}

此次次版本发布**不**引入破坏性变更。主要功能新增请查看上游 [OpenAI Agents SDK v0.11.0 发布说明](https://github.com/openai/openai-agents-python/releases/tag/v0.11.0)（本 fork 所追踪的上游版本线）。
