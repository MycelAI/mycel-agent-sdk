...
## 使用同一会话恢复中断运行 {#resuming-interrupted-runs-with-the-same-session}
...
## 控制历史与新输入的合并方式 {#control-how-history-and-new-input-merge}
...
## 限制检索历史 {#limiting-retrieved-history}
...
### 选择内置会话实现 {#choose-a-built-in-session-implementation}
...
如果你的智能体运行使用 `ModelSettings(store=False)`，Responses API 不会保留最后一次响应供后续查找。在这种无状态设置下，默认 `"auto"` 模式会回退为基于输入的压缩，而不是依赖 `previous_response_id`。完整示例见 [`examples/memory/compaction_session_stateless_example.py`](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/memory/compaction_session_stateless_example.py)。

#### 自动压缩可能阻塞流式传输 {#auto-compaction-can-block-streaming}
...
-   完整配置流程（含本地组件与故障排查）请见 [`examples/memory/dapr_session_example.py`](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/memory/dapr_session_example.py)。
