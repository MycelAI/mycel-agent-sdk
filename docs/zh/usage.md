...
## 从一次运行中访问使用情况 {#accessing-usage-from-a-run}
...
### 在第三方适配器中启用使用情况追踪 {#enabling-usage-with-third-party-adapters}
...
- 使用与 OpenAI 兼容的网关或部分第三方后端时，默认可能不上报用量，因此通常也需要 `ModelSettings(include_usage=True)`。

请查看 Models 指南中[第三方适配器](models/index.md#third-party-adapters)章节的适配器说明，并验证你计划部署的具体提供方后端。

## 按请求追踪使用情况 {#per-request-usage-tracking}
...
## 在会话中访问使用情况 {#accessing-usage-with-sessions}
...
## 在 hooks 中使用使用情况 {#using-usage-in-hooks}
