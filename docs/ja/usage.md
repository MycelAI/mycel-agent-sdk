...
## 実行からの使用量アクセス {#accessing-usage-from-a-run}
...
### サードパーティアダプターでの使用量有効化 {#enabling-usage-with-third-party-adapters}
...
- OpenAI 互換ゲートウェイや一部のサードパーティバックエンドでは、既定で使用量が返らないことがあり、その場合も `ModelSettings(include_usage=True)` が必要になることがよくあります。
...
## リクエストごとの使用量追跡 {#per-request-usage-tracking}
...
## セッションでの使用量アクセス {#accessing-usage-with-sessions}
...
## フックでの使用量活用 {#using-usage-in-hooks}
