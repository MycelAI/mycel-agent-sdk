...
## サーバーサイド realtime session の作成 {#create-a-server-side-realtime-session}

### 1. Realtime コンポーネントのインポート {#1-import-the-realtime-components}
...
### 3. runner の設定 {#3-configure-the-runner}
...
### 4. session の開始と入力の送信 {#4-start-the-session-and-send-input}
...
## このクイックスタートに含まれない内容 {#what-this-quickstart-does-not-include}

-   マイク入力とスピーカー再生のコード。[`examples/realtime`](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/realtime) の realtime コード例を参照してください。
...
-   [`examples/realtime`](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/realtime) のコード例を確認してください。