...
### 基本の Hosted MCP ツール {#basic-hosted-mcp-tool}
...
### Hosted MCP 結果のストリーミング {#streaming-hosted-mcp-results}
...
### 任意の承認フロー {#optional-approval-flows}
...
### コネクタをバックエンドとする Hosted サーバー {#connector-backed-hosted-servers}
...
ストリーミング、承認、コネクタを含む完全動作する Hosted ツールのサンプルは、[`examples/hosted_mcp`](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/hosted_mcp) にあります。
...
### MCP ツール出力: テキストと画像 {#mcp-tool-outputs-text-and-images}
...
## 共通サーバー機能 {#common-server-capabilities}
...
- [examples/mcp](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/mcp) – 実行可能な stdio 、 SSE 、 Streamable HTTP サンプル。
- [examples/hosted_mcp](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/hosted_mcp) – 承認とコネクタを含む完全な hosted MCP デモ。