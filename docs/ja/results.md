...
## 適切な結果サーフェスの選択 {#choose-the-right-result-surface}
...
## 入力、次ターン履歴、new items {#input-next-turn-history-and-new-items}
...
## 会話の継続または再開 {#continue-or-resume-the-conversation}
...
### 割り込みと実行状態 {#interruptions-and-run-state}
...
## Agent-as-tool メタデータ {#agent-as-tool-metadata}
...
## ストリーミングライフサイクルと診断 {#streaming-lifecycle-and-diagnostics}
