...
#### Responses WebSocket トランスポート (任意ヘルパー) {#responses-websocket-transport-optional-helper}
...
##### パターン 1: session helper なし (動作可) {#pattern-1-no-session-helper-works}
...
##### パターン 2: `responses_websocket_session()` を使用 (マルチターン再利用に推奨) {#pattern-2-use-responses_websocket_session-recommended-for-multi-turn-reuse}
...
#### 共通の run_config カテゴリー {#common-run-config-categories}

`RunConfig` を使うと、各エージェント定義を変更せずに、単一実行の挙動を上書きできます。

##### model / provider / session の既定値 {#model-provider-and-session-defaults}
...
##### ガードレール / ハンドオフ / model 入力整形 {#guardrails-handoffs-and-model-input-shaping}
...
##### ツール承認とツールエラー挙動 {#tool-approval-and-tool-error-behavior}
...
#### RunConfig 詳細 {#run-config-details}
...
## 状態と会話管理 {#state-and-conversation-management}
...
### 会話 / チャットスレッド {#conversationschat-threads}
...
#### 手動の会話管理 {#manual-conversation-management}
...
#### サーバー管理会話 {#server-managed-conversations}
...
### call model input filter {#call-model-input-filter}
...
## Durable execution 連携と human-in-the-loop {#durable-execution-integrations-and-human-in-the-loop}
