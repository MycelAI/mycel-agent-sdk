...
##### 패턴 1: 세션 헬퍼 미사용(작동함) {#pattern-1-no-session-helper-works}
...
##### 패턴 2: `responses_websocket_session()` 사용(멀티턴 재사용 권장) {#pattern-2-use-responses_websocket_session-recommended-for-multi-turn-reuse}
...
#### 공통 실행 구성 카테고리 {#common-run-config-categories}

각 에이전트 정의를 변경하지 않고 단일 실행의 동작을 재정의하려면 `RunConfig`를 사용하세요

##### 모델, provider, 세션 기본값 {#model-provider-and-session-defaults}
...
##### 가드레일, 핸드오프, 모델 입력 형태 조정 {#guardrails-handoffs-and-model-input-shaping}
...
##### 도구 승인 및 도구 오류 동작 {#tool-approval-and-tool-error-behavior}
...
#### 세션을 이용한 자동 대화 관리 {#automatic-conversation-management-with-sessions}
