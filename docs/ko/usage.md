...
### 서드파티 어댑터에서 사용량 활성화 {#enabling-usage-with-third-party-adapters}
...
- OpenAI 호환 게이트웨이나 일부 서드파티 백엔드는 기본적으로 사용량을 보고하지 않을 수 있어, `ModelSettings(include_usage=True)`가 자주 필요합니다.
