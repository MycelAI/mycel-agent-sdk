...
search:
  exclude: true
---
# 릴리스 프로세스/변경 로그 {#release-processchangelog}
...
## 호환성이 깨지는 변경 로그 {#breaking-change-changelog}
...
- Chat Completions 통합에서 이제 `should_replay_reasoning_content`를 통해 추론 콘텐츠 재생을 선택적으로 활성화할 수 있어, 서드파티 Chat Completions 어댑터(예: DeepSeek)의 제공자별 추론/도구 호출 연속성이 개선됩니다
...
이 마이너 릴리스는 **호환성이 깨지는 변경**을 도입하지 않습니다. 주요 기능 추가는 업스트림 [OpenAI Agents SDK v0.12.0 릴리스 노트](https://github.com/openai/openai-agents-python/releases/tag/v0.12.0)(이 포크가 추적하는 계열)를 참고하세요.

### 0.11.0 {#0110}

이 마이너 릴리스는 **호환성이 깨지는 변경**을 도입하지 않습니다. 주요 기능 추가는 업스트림 [OpenAI Agents SDK v0.11.0 릴리스 노트](https://github.com/openai/openai-agents-python/releases/tag/v0.11.0)(이 포크가 추적하는 계열)를 참고하세요.
