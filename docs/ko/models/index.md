...
#### provider 또는 실행 단위 설정 {#provider-or-run-level-setup}
...
#### `MultiProvider`를 사용한 고급 라우팅 {#advanced-routing-with-multiprovider}
...
-   websocket 전송을 활성화한 뒤 [`Runner.run_streamed()`][agents.run.Runner.run_streamed]를 바로 사용할 수 있습니다. 여러 턴 워크플로에서 동일 websocket 연결을 턴 간(및 중첩된 agent-as-tool 호출 간) 재사용하려면 [`responses_websocket_session()`][agents.responses_websocket_session] 헬퍼를 권장합니다. [에이전트 실행](../running_agents.md) 가이드와 [`examples/basic/stream_ws.py`](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/basic/stream_ws.py)를 참고하세요

## OpenAI 이외 모델 {#non-openai-models}

OpenAI 이외 provider가 필요하면 SDK의 내장 provider 통합 지점부터 시작하세요. 많은 설정에서 서드파티 어댑터 없이도 충분합니다. 각 패턴의 예시는 [examples/model_providers](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/model_providers/)에 있습니다

### OpenAI 이외 provider 통합 방법 {#ways-to-integrate-non-openai-providers}
...
1. [`set_default_openai_client`][agents.set_default_openai_client]는 LLM 클라이언트로 `AsyncOpenAI` 인스턴스를 전역적으로 사용하려는 경우 유용합니다. LLM provider가 OpenAI 호환 API 엔드포인트를 제공하고 `base_url` 및 `api_key`를 설정할 수 있는 경우에 해당합니다. 구성 가능한 예시는 [examples/model_providers/custom_example_global.py](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/model_providers/custom_example_global.py)를 참고하세요
2. [`ModelProvider`][agents.models.interface.ModelProvider]는 `Runner.run` 수준에서 적용됩니다. 이를 통해 "이 실행의 모든 에이전트에 커스텀 model provider를 사용"하도록 지정할 수 있습니다. 구성 가능한 예시는 [examples/model_providers/custom_example_provider.py](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/model_providers/custom_example_provider.py)를 참고하세요
3. [`Agent.model`][agents.agent.Agent.model]은 특정 Agent 인스턴스에 모델을 지정할 수 있게 해줍니다. 이를 통해 서로 다른 에이전트에 서로 다른 provider를 조합해 사용할 수 있습니다. 구성 가능한 예시는 [examples/model_providers/custom_example_agent.py](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/model_providers/custom_example_agent.py)를 참고하세요
...
더 자세한 예시는 [`examples/basic/retry.py`](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/basic/retry.py) 및 [OpenAI Chat Completions 어댑터 재시도 예시](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/basic/retry_openai_chatcompletions.py)를 참고하세요

## OpenAI 이외 provider 문제 해결 {#troubleshooting-non-openai-providers}
...
2. [`OpenAIChatCompletionsModel`][agents.models.openai_chatcompletions.OpenAIChatCompletionsModel] 사용. 예시는 [여기](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/model_providers/)에 있습니다
...
## provider 간 모델 혼합 {#mixing-models-across-providers}
...
## 서드파티 어댑터 {#third-party-adapters}

SDK 내장 provider 통합만으로 부족할 때만 서드파티 어댑터를 쓰세요. 이 SDK에서 OpenAI 모델만 쓴다면 Any-LLM 대신 내장 [`OpenAIResponsesModel`][agents.models.openai_responses.OpenAIResponsesModel] 경로를 우선하세요. 멀티 벤더 라우팅은 OpenAI 호환 HTTP 게이트웨이와 [`OpenAIChatCompletionsModel`][agents.models.openai_chatcompletions.OpenAIChatCompletionsModel] 및 `AsyncOpenAI(base_url=...)`를 사용합니다([게이트웨이 모델 프로바이더](../gateway_model_provider.md) 참고). 서드파티 어댑터는 내장 경로로 충족되지 않는 어댑터 관리 커버리지가 필요할 때를 위한 것입니다. SDK는 베타로 Any-LLM을 포함합니다.

### Any-LLM {#any-llm}

Any-LLM은 best-effort 베타로, Any-LLM이 관리하는 커버리지나 라우팅이 필요할 때 제공됩니다.

업스트림 경로에 따라 Responses API, Chat Completions 호환 API 또는 provider별 호환 계층을 쓸 수 있습니다.

`mycel-agent-sdk[any-llm]`를 설치한 뒤 [`examples/model_providers/any_llm_auto.py`](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/model_providers/any_llm_auto.py) 또는 [`examples/model_providers/any_llm_provider.py`](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/model_providers/any_llm_provider.py)부터 시작하세요. [`MultiProvider`][agents.MultiProvider]에서 `any-llm/...` 모델명을 쓰거나 `AnyLLMModel`을 직접 만들거나 실행 범위에서 `AnyLLMProvider`를 쓸 수 있습니다. 모델 표면을 고정하려면 `AnyLLMModel` 생성 시 `api="responses"` 또는 `api="chat_completions"`를 전달하세요.

Any-LLM은 여전히 서드파티 어댑터 계층이므로 의존성과 기능 격차는 업스트림 Any-LLM이 정의합니다. 사용량은 업스트림이 반환하면 전달되지만 스트리밍 Chat Completions에서는 `ModelSettings(include_usage=True)`가 필요할 수 있습니다. structured outputs, 도구 호출, 사용량 보고, Responses 전용 동작에 의존하면 배포할 정확한 백엔드를 검증하세요.

### OpenAI 호환 게이트웨이 {#openai-compatible-gateway}

별도 프로세스(예: Rust **mycel-gateway**)가 OpenAI Chat Completions API를 노출하고 라우팅할 때 [`OpenAIChatCompletionsModel`][agents.models.openai_chatcompletions.OpenAIChatCompletionsModel]과 `AsyncOpenAI(base_url="http://localhost:8080/v1", api_key=...)`를 사용하세요. 자세한 내용은 [게이트웨이 모델 프로바이더](../gateway_model_provider.md)와 [`examples/model_providers/gateway_chatcompletions.py`](https://github.com/MycelAI/mycel-agent-sdk/tree/main/examples/model_providers/gateway_chatcompletions.py)를 참고하세요.